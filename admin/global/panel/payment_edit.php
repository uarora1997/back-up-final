    ...
    <div class="slidePanel-inner">
        <form method="post" id="sidePanel_form" data-ajax-action="paymentEdit">
                ...
                if($folder == "paypal") {
                    ...
                    <?php
                }elseif($folder == "coinbase"){
                    ?>
                    <div class="form-group">
                        <h4><?php _e('Integration Steps:') ?></h4>
                        <ol>
                            <li><?php _e('Go to <a href="https://commerce.coinbase.com/dashboard">Coinbase Dashboard</a> and Login.') ?></li>
                            <li><?php _e('Go to the <a href="https://commerce.coinbase.com/dashboard/settings">Settings</a> page.') ?></li>
                            <li><?php _e('Copy the <strong>API key</strong> and paste here.') ?></li>
                            <li><?php _e('Scroll down to the Webhook subscriptions section, click on the <strong>Add an endpoint</strong> button.') ?></li>
                            <li><?php _e('In the <strong>Endpoint URL</strong> field, paste the Webhook Url') ?> <code><?php echo _esc($config['site_url'], false).'webhook/coinbase'?></code> and save it.</li>
                            <li><?php _e('Copy the <strong>secret</strong> key and paste below in <strong>Webhook Secret</strong> field.') ?></li>
                        </ol>
                    </div>
                    <?php
                } elseif($folder == "paddle"){
                    ?>
                    <div class="form-group">
                        <h4><?php _e('Integration Steps:') ?></h4>
                        <ol>
                            <li><?php _e('Go to <a href="https://vendors.paddle.com">Paddle Vendors</a> and Login.') ?></li>
                            <li><?php _e('Go to the <strong>Developers Tools</strong> -> <strong>Authentication</strong> page.') ?></li>
                            <li><?php _e('Copy your <strong>Vendor ID</strong> and <strong>API Key</strong> and paste below.') ?></li>
                            <li><?php _e('Go to Paddle <strong>Developers Tools</strong> -> <strong>Public Key</strong> and copy your public key and paste below.') ?></li>
                        </ol>
                    </div>
                    <?php
                }
            ...
            if($folder == "flutterwave"){
                ?>
                <div class="form-group">
                    <label><?php _e('Flutterwave API Key') ?></label>
                        <input name="flutterwave_api_key" type="text" class="form-control" value="<?php echo get_option('flutterwave_api_key')?>">
                </div>

                <div class="form-group">
                    <label><?php _e('Flutterwave Secret Key') ?></label>
                        <input name="flutterwave_secret_key" type="text" class="form-control" value="<?php echo get_option('flutterwave_secret_key')?>">
                </div>
                <?php
            }
            ?>
            ...
            if($folder == "yoomoney"){
                ?>
                <div class="form-group">
                    <label><?php _e('YooMoney Shop Id') ?></label>
                    <input name="yoomoney_shop_id" type="text" class="form-control" value="<?php echo get_option('yoomoney_shop_id')?>">
                </div>
                <div class="form-group">
                    <label><?php _e('YooMoney Secret Key') ?></label>
                    <input name="yoomoney_secret_key" type="text" class="form-control" value="<?php echo get_option('yoomoney_secret_key')?>">
                </div>
                <?php
            }
            ?>
            ...
            if($folder == "coinbase"){
                ...
                <div class="form-group">
                    ...
                    <input name="coinbase_api_key" type="text" class="form-control" value="<?php echo get_option('coinbase_api_key')?>">
                </div>
                <div class="form-group">
                    <label><?php _e('Webhook Secret') ?></label>
                    <input name="coinbase_webhook_secret" type="text" class="form-control" value="<?php echo get_option('coinbase_webhook_secret')?>">
                </div>
                <div class="form-group">
                    <label for="coinbase_webhook"><?php _e('WebHook Url') ?></label>
                    <input type="text" id="coinbase_webhook" class="form-control" value="<?php echo _esc($config['site_url'], false).'webhook/coinbase'?>" readonly>
                </div>
                <?php
            }
            ?>
            <?php
            if($folder == "mercadopago"){
                ?>
                <div class="form-group">
                    <label><?php _e('Access Token') ?></label>
                    <input name="mercadopago_access_token" type="text" class="form-control" value="<?php echo get_option('mercadopago_access_token')?>">
                </div>
                <?php
            }
            ?>
            ...
            if($folder == "paddle"){
                ?>
                <div class="form-group">
                    <label for="paddle_sandbox_mode"><?php _e('Live Mode/Sandbox Mode') ?></label>
                    <select name="paddle_sandbox_mode" id="paddle_sandbox_mode" class="form-control">
                        <option value="test" <?php if(get_option('paddle_sandbox_mode') == 'test') echo "selected"; ?>><?php _e('Sandbox Test Mode') ?></option>
                        <option value="live" <?php if(get_option('paddle_sandbox_mode') == 'live') echo "selected"; ?>><?php _e('Live Mode') ?></option>
                    </select>
                </div>
                <div class="form-group">
                    <label><?php _e('Vendor ID') ?></label>
                    <input name="paddle_vendor_id" type="text" class="form-control" value="<?php echo get_option('paddle_vendor_id')?>">
                </div>
                <div class="form-group">
                    <label><?php _e('API Key') ?></label>
                    <input name="paddle_api_key" type="text" class="form-control" value="<?php echo get_option('paddle_api_key')?>">
                </div>
                <div class="form-group">
                    <label><?php _e('Public Key') ?></label>
                    <textarea name="paddle_public_key" class="form-control" rows="2"><?php echo get_option('paddle_public_key')?></textarea>
